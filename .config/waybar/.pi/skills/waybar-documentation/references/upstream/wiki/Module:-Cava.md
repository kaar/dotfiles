> [!NOTE]
> This page is **auto-generated from [`man/waybar-cava.5.scd`](https://github.com/Alexays/Waybar/blob/master/man/waybar-cava.5.scd)** on the `master` branch.
...
| **data_format** | string | ascii | Raw data format. Can be 'binary' or 'ascii'. Raw frontend only. |
| **raw_target** | string |  | Raw output target. A fifo will be created if target does not exist. Raw frontend only. |
...
| **min-length** | integer |  | Requested width of the GLSL widget in pixels. If not set, falls back to **max-length**, then **sdl_width**. GLSL only. |
| **max-length** | integer |  | Fallback width of the GLSL widget if **min-length** is not set. GLSL only. |
...
| **gradient** | integer | 0 | Enable gradient mode (0 = off, 1 = on). GLSL only. Can also be set in the **\[color\]** section of the cava configuration file. |
| **gradient_count** | integer | 0 | Number of gradient colors (up to 8). GLSL only. Can also be set in the **\[color\]** section of the cava configuration file. |
