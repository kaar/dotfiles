...
## CFFI module config_entries behavior

When Waybar passes configuration values to CFFI modules, they are converted into strings before being exposed.

Current behavior:
- String values are typically retrieved using `asString()`
- Non-string values are serialized using `toStyledString()`

Note:
- `toStyledString()` produces human-readable output and may introduce formatting artifacts (such as escaped quotes or comment placement)
- This can lead to inconsistencies when parsing values inside CFFI modules

This behavior is not strictly standardized and may change in the future.