> [!NOTE]
> This page is **auto-generated from [`man/waybar-gps.5.scd`](https://github.com/Alexays/Waybar/blob/master/man/waybar-gps.5.scd)** on the `master` branch.
...
|:--:|:--:|:--:|:--:|
| **format** | string | {mode} | The text format. |
| **tooltip** | bool | true | Option to disable tooltip on hover. |
| **tooltip-format** | string | The value of **format** | The text format of the tooltip. |
| **interval** | integer | 5 | The interval in which the GPS information gets polled (e.g. current speed). Significant updates (e.g. the current fix mode) are updated immediately. |
| **hide-disconnected** | bool | true | Defines if the module should be hidden if there is no GPS receiver. |
| **hide-no-fix** | bool | false | Defines if the module should be hidden if there is no GPS fix. |
